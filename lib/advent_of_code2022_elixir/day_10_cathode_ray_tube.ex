defmodule AdventOfCode2022Elixir.Day10CathodeRayTube do
  ...
  def crt(input, cycles \\ 240) do
    register = RegisterX.new(parse(input))

    1..cycles
    |> Enum.reduce({register, []}, fn cycle, {register, output} ->
      position = cycle - 1
      position = rem(position, 40)

      new_output = if abs(register.value - position) <= 1, do: "#", else: "."
      new_register = RegisterX.cycle(register)

      {new_register, [new_output | output]}
    end)
    |> elem(1)
    |> Enum.reverse()
    |> Enum.chunk_every(40)
